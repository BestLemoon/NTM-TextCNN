...
parser.add_argument('--no_above', type=float, default=0.0134,
...
parser.add_argument('--model_path', type=str, default='./ckpt/GSM_weibotest_tp20_2023-04-07-15-51_ep550.ckpt',
                    help='Load model for inference from this path')
parser.add_argument('--save_dir', type=str, default='./result', help='Save inference result')
parser.add_argument('--model_name', type=str, default='GSM', help='Neural Topic Model name')
parser.add_argument('--test_path', type=str, default='./test.txt', help='Test set path')
...
def main():
    ...
    # load test dataset
    testSet = TestData(dictionary=dictionary, lang="zh", txtPath=test_path, no_below=no_below, no_above=no_above,
                       use_tfidf=use_tfidf)
    ...
    with open(test_path, "r",encoding='utf-8') as f:
        for i in range(10):
            print(f.readline(), " + ".join(["topic{}*{}".format(j, round(w, 3)) for j, w in
                                            sorted(enumerate(infer_topics[i]), key=lambda x: x[1], reverse=True)]))

    # save results
    with open(save_dir + "/inference_result_{}_{}.json".format(model_name,
